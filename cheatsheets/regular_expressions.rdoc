...
=== Ruby Regular Expressions
    ...
    # modified version of Ryan Davis' cheat sheet at http://www.zenspider.com/Languages/Ruby/QuickRef.html#11
    ...
    # to create a regular expression, just surround it with slashes
    /regex/       # => /regex/
    /regex/.class # => Regexp


    # if you have a bunch of / in your regular expression, you would have to escape them, so that they don't look like the end of the regular expression
    # so you can also create a regular expression with %r followed by a delimiter (just like %w for Arrays of Strings)
    %r(regex)     # => /regex/
    %r[regex]     # => /regex/
    %r{regex}     # => /regex/
    %r(/usr/bin)  # => /\/usr\/bin/
    ...
    # =====  Ways to Use regular expressions (simple)  =====
    
    str = "This is a cheat sheet for regular expressions."
    ...
    # substitute the first match with sub, and every match with global sub (apply the regular expression "globally" to the String -- g is a flag in some other languages)
    ...
    # use the match as a delimiter, and find everything in between
    str.split(/b.b/)  # => ["", "bling ", "bling ", "bles"]

    # These don't directly work on Symbols, but if you invoke the get regular expression method on an Array
    # it will convert them to Symbols before checking your regular expression
    # This will go through each element, and if the regular expression matches it, that element will be in the return array
    ...
    # You can append a flag to the end of a regular expression to alter its behaviour
    # /i         case insensitive
    ...
    # the plus does the same thing as the splat, but matches 1 or more instead of 0 or more
    ...
    # note that in this example, we don't need multi-line because the match itself is not going across multiple lines
    str = "Hello my\ndarling how\nare you?"
    str.scan(/^./)      # => ["H", "d", "a"]
    str.scan(/.$/)      # => ["y", "w", "?"]
    str.scan(/.$.^./)   # => []
    ...
    # to match the break between a word and non-word boundary, use \b
